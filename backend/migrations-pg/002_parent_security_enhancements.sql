...
CREATE TABLE IF NOT EXISTS parent_notifications (
    ...
    child_id VARCHAR(36),
    type VARCHAR(50) CHECK (role IN ('info', 'warning', 'urgent', 'success')) DEFAULT 'info',
...
CREATE TABLE IF NOT EXISTS child_daily_reports (
    ...
    arrival_time TIME,
    departure_time TIME,
    mood_morning VARCHAR(50) CHECK (role IN ('happy', 'sad', 'tired', 'excited', 'anxious')) DEFAULT 'happy',
    mood_afternoon VARCHAR(50) CHECK (role IN ('happy', 'sad', 'tired', 'excited', 'anxious')) DEFAULT 'happy',
    meals_breakfast BOOLEAN DEFAULT FALSE,
    meals_lunch BOOLEAN DEFAULT FALSE,
    meals_snack_morning BOOLEAN DEFAULT FALSE,
    meals_snack_afternoon BOOLEAN DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS parent_child_media (
    ...
    file_type VARCHAR(50) CHECK (role IN ('image', 'video')) NOT NULL,
    file_size INT,
...
CREATE TABLE IF NOT EXISTS parent_feedback (
    ...
    parent_id VARCHAR(36) NOT NULL,
    child_id VARCHAR(36),
    feedback_type VARCHAR(50) CHECK (role IN ('suggestion', 'complaint', 'compliment', 'question')) NOT NULL,
    subject VARCHAR(255) NOT NULL,
    message TEXT NOT NULL,
    priority VARCHAR(50) CHECK (role IN ('low', 'medium', 'high')) DEFAULT 'medium',
    status VARCHAR(50) CHECK (role IN ('pending', 'in_progress', 'resolved', 'closed')) DEFAULT 'pending',
    response TEXT,
