...
CREATE TABLE expenses (
    ...
    payment_mode VARCHAR(50) CHECK (role IN ('cheque','cash','UPI','online','RTGS')) NOT NULL,
    vendor VARCHAR(128),
    receipt_image_url VARCHAR(255),
    created_by VARCHAR(36) NOT NULL,
    raised_by_role VARCHAR(50) CHECK (role IN ('admin','owner','financial_manager')) NOT NULL,
    approved_by VARCHAR(36),
    status VARCHAR(50) CHECK (role IN ('pending','approved','rejected')) DEFAULT 'pending',
    approval_notes TEXT,
    recurring VARCHAR(50) CHECK (role IN ('No','Yes')) DEFAULT 'No',
    recurring_type VARCHAR(50) CHECK (role IN ('monthly','quarterly','yearly')),
    next_due_date DATE,
