...
ALTER TABLE digital_portfolios ADD COLUMN capture_method VARCHAR(50) CHECK (role IN ('camera', 'upload', 'import')) DEFAULT 'upload' COMMENT 'How the media was captured';
ALTER TABLE digital_portfolios ADD COLUMN processing_status VARCHAR(50) CHECK (role IN ('pending', 'processing', 'completed', 'failed')) DEFAULT 'completed' COMMENT 'Image processing status';
...
CREATE TABLE IF NOT EXISTS image_processing_jobs (
    ...
    job_type VARCHAR(50) CHECK (role IN ('thumbnail', 'compress', 'watermark', 'crop')) NOT NULL,
    job_status VARCHAR(50) CHECK (role IN ('queued', 'processing', 'completed', 'failed')) DEFAULT 'queued',
