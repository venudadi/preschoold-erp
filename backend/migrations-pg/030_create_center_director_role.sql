...
CREATE TABLE IF NOT EXISTS budget_approvals (
    ...
    status VARCHAR(50) CHECK (role IN ('pending', 'approved', 'rejected', 'revision_required')) DEFAULT 'pending',
    approval_notes TEXT,
    priority VARCHAR(50) CHECK (role IN ('low', 'medium', 'high', 'urgent')) DEFAULT 'medium',
...
CREATE TABLE IF NOT EXISTS staff_schedules (
    ...
    status VARCHAR(50) CHECK (role IN ('scheduled', 'confirmed', 'absent', 'substitute', 'overtime')) DEFAULT 'scheduled',
    notes TEXT,
...
CREATE TABLE IF NOT EXISTS operational_kpis (
    ...
    measurement_period VARCHAR(50) CHECK (role IN ('daily', 'weekly', 'monthly', 'quarterly', 'yearly')) NOT NULL,
    category ENUM(
        'enrollment',
        'attendance',
        'financial',
...
CREATE TABLE IF NOT EXISTS incident_reports (
    ...
    severity VARCHAR(50) CHECK (role IN ('low', 'medium', 'high', 'critical')) NOT NULL,
    ...
    status VARCHAR(50) CHECK (role IN ('reported', 'under_review', 'resolved', 'closed')) DEFAULT 'reported',
...
CREATE TABLE IF NOT EXISTS parent_feedback (
    ...
    status VARCHAR(50) CHECK (role IN ('new', 'acknowledged', 'in_progress', 'resolved', 'closed')) DEFAULT 'new',
...
CREATE TABLE IF NOT EXISTS center_policies (
    ...
    status VARCHAR(50) CHECK (role IN ('draft', 'pending_approval', 'active', 'archived')) DEFAULT 'draft',
    created_by VARCHAR(36) NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ,
...
CREATE TABLE IF NOT EXISTS staff_performance (
    ...
    evaluation_type VARCHAR(50) CHECK (role IN ('probation', 'annual', 'mid_year', 'project_based', 'disciplinary')) NOT NULL,
    overall_rating VARCHAR(50) CHECK (role IN ('outstanding', 'exceeds_expectations', 'meets_expectations', 'below_expectations', 'unsatisfactory')) NOT NULL,
    ...
    staff_comments TEXT,
    status VARCHAR(50) CHECK (role IN ('draft', 'completed', 'acknowledged', 'disputed')) DEFAULT 'draft',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ,
...
ADD COLUMN IF NOT EXISTS management_level VARCHAR(50) CHECK (role IN ('junior', 'senior', 'executive')) DEFAULT 'junior' COMMENT 'Management level for role hierarchy';
