...
ALTER TABLE children ADD COLUMN status VARCHAR(50) CHECK (role IN ('active', 'paused', 'left')) NOT NULL DEFAULT 'active';
...
CREATE TABLE IF NOT EXISTS student_pause_history (
    ...
    status VARCHAR(50) CHECK (role IN ('active', 'completed', 'cancelled')) DEFAULT 'active',
