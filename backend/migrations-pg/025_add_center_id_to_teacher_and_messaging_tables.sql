...
ALTER TABLE lesson_plans
  ADD COLUMN center_id VARCHAR(36) AFTER classroom_id,
  ADD CONSTRAINT fk_lesson_plans_center FOREIGN KEY (center_id) REFERENCES centers(id);
CREATE INDEX idx_lesson_plans_center_id ON lesson_plans(center_id);

-- Add center_id to assignments
ALTER TABLE assignments
  ADD COLUMN center_id VARCHAR(36) AFTER classroom_id,
  ADD CONSTRAINT fk_assignments_center FOREIGN KEY (center_id) REFERENCES centers(id);
CREATE INDEX idx_assignments_center_id ON assignments(center_id);

-- Add center_id to assignment_submissions
ALTER TABLE assignment_submissions
  ADD COLUMN center_id VARCHAR(36) AFTER assignment_id,
  ADD CONSTRAINT fk_assignment_submissions_center FOREIGN KEY (center_id) REFERENCES centers(id);
CREATE INDEX idx_assignment_submissions_center_id ON assignment_submissions(center_id);

-- Add center_id to messages
ALTER TABLE messages
  ADD COLUMN center_id VARCHAR(36) AFTER thread_id,
  ADD CONSTRAINT fk_messages_center FOREIGN KEY (center_id) REFERENCES centers(id);
...
ALTER TABLE message_threads
  ...
  ADD CONSTRAINT fk_message_threads_center FOREIGN KEY (center_id) REFERENCES centers(id);
