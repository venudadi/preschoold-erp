...
CREATE TABLE IF NOT EXISTS staff_profiles (
    ...
    contract_type VARCHAR(50) CHECK (role IN ('full-time', 'part-time', 'contract', 'temporary')) NOT NULL,
...
CREATE TABLE IF NOT EXISTS staff_schedules (
    ...
    center_id VARCHAR(36) NOT NULL,
    day_of_week VARCHAR(50) CHECK (role IN ('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday')) NOT NULL,
...
CREATE TABLE IF NOT EXISTS staff_attendance (
    ...
    check_in TIMESTAMP,
    check_out TIMESTAMP,
    status VARCHAR(50) CHECK (role IN ('present', 'absent', 'late', 'half-day', 'on-leave')) NOT NULL,
    remarks TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ,
    FOREIGN KEY (staff_id) REFERENCES staff_profiles(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS staff_leaves (
    ...
    staff_id VARCHAR(36) NOT NULL,
    center_id VARCHAR(36) NOT NULL,
    leave_type VARCHAR(50) CHECK (role IN ('casual', 'sick', 'annual', 'maternity', 'paternity', 'unpaid')) NOT NULL,
...
CREATE TABLE IF NOT EXISTS staff_documents (
    ...
    expiry_date DATE,
    verification_status VARCHAR(50) CHECK (role IN ('pending', 'verified', 'rejected')) DEFAULT 'pending',
...
CREATE TABLE IF NOT EXISTS staff_performance_reviews (
    ...
    status VARCHAR(50) CHECK (role IN ('draft', 'published', 'acknowledged')) DEFAULT 'draft',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ,
    FOREIGN KEY (staff_id) REFERENCES staff_profiles(id) ON DELETE CASCADE,
    FOREIGN KEY (center_id) REFERENCES centers(id) ON DELETE CASCADE,
